...
SIGMA = 0.10                 # mutation strength or step size
...
# _______________________Main Activation function__________________________________
def relu(x):
    s = np.where(x < 0, 0, x)
    return s
...
# ______________________Population Inicialization___________________________________________
...
if __name__ == "__main__":
    ...
    for i in range(50):
        s = env.reset()
        ep_rr = 0.
        for _ in range(CONFIG['ep_max_step'] * 3):
            env.render()

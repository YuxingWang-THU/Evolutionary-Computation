...
def configure_rate(before, after, pop_size):
    count = after - before
    return sum(count > 0) / pop_size
...
def make_kids(pop, kid):
    ...
    before_fitness = get_fitness(F(pop['DNA'][:, 0], pop['DNA'][:, 1]))
    for kv, ks in zip(kids['DNA'], kids['mut_strength']):
        ...
        if not auto_adapt:
            ...
            if theta <= 0.2:
                ks[:] = ks + ((np.random.rand(*ks.shape) - 0.5) * C * C)
                kv += ks * np.random.randn(*kv.shape) * C * C
            else:
                ks[:] = ks + ((np.random.rand(*ks.shape) - 0.5) / (C * C))
                kv += ks * np.random.randn(*kv.shape) / (C * C)
        ...
        after_fitness = get_fitness(F(kids['DNA'][:, 0], kids['DNA'][:, 1]))
    return kids, before_fitness, after_fitness
...
if __name__ == '__main__':
    ...
    for _ in range(N_GENERATIONS):
        ...
        # point.remove()
        # ES part
        kids, before_fitness, after_fitness = make_kids(pop, N_KID)
        pop = kill_bad(pop, kids)  # keep some good parent for elitism
        if auto_adapt:
            if G < 2:
                fit_rate.append(configure_rate(before_fitness, after_fitness, POP_SIZE))
                G += 1
            else:
                fit_rate.append(configure_rate(before_fitness, after_fitness, POP_SIZE))
                theta = sum(fit_rate) / 2
    ...
    plt.plot(np.linspace(0, 100, len(theta_value)), theta_value)
    plt.ioff()
    plt.show()